...
class SphinxClient:
    def __init__(self):
        ...
        self._socket = None
        self._offset = (
            0  # how much records to seek from result-set start (default is 0)
        )
        ...
        self._sort = (
            SPH_SORT_RELEVANCE  # match sorting mode (default is SPH_SORT_RELEVANCE)
        )
        ...
        self._groupsort = (
            "@group desc"  # group-by sorting clause (to sort groups in result set with)
        )
        ...
        self._maxquerytime = (
            0  # max query time, milliseconds (default is 0, do not limit)
        )
    ...
    def AddQuery(self, query, index="*", comment=""):
        ...
        # build request
        req = [
            pack(">5L", self._offset, self._limit, self._mode, self._ranker, self._sort)
        ]
            ...
            while count > 0 and p < max_:
                ...
                for i in range(len(attrs)):
                    if attrs[i][1] == SPH_ATTR_FLOAT:
                        match["attrs"][attrs[i][0]] = unpack(">f", response[p: p + 4])[
                            0
                        ]
                    elif attrs[i][1] == SPH_ATTR_BIGINT:
                        match["attrs"][attrs[i][0]] = unpack(">q", response[p: p + 8])[
                            0
                        ]
                        p += 4
                        ...
                        for n in range(0, nvals, 1):
                            match["attrs"][attrs[i][0]].append(
                                unpack(">L", response[p: p + 4])[0]
                            )
                            p += 4
                        p -= 4
                    else:
                        match["attrs"][attrs[i][0]] = unpack(">L", response[p: p + 4])[
                            0
                        ]
                    p += 4
